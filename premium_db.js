...
async function fetchAndSave() {
  ...
  for (let i = 0; i < symbolsLength; i++) {
    ...
    const regularMarketPrice = await db.createQuery('/quote/regularMarketPrice | /quote/regularMarketPrice', symbols[i])

    for await (const doc of regularMarketPrice.stream()) {
      console.log(`Found ${doc}`);
    }
    const q = db.createQuery('/* | /options/0/calls/* + /quote/regularMarketPrice', symbols[i]) // [hasMiniOptions = :hasMiniOptions]
